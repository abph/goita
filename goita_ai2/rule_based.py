...
class RuleBasedAgent:
    ...
    def _would_lose_if_pass(self, state, player: str) -> bool:
        """この局面で pass すると（ルール上）即座に試合が終了し、相手側の勝ちになるなら True。
        ※ state.apply_pass の仕様に依存するため、コピーに適用して finished/winner で判定する。
        """
        try:
            s = self._apply_action_on_copy(state, player, ("pass", None, None))
        except Exception:
            return False
        if not getattr(s, "finished", False):
            return False
        winner = getattr(s, "winner", None)
        if winner is None:
            return False
        return not self._same_team(winner, player)
    ...
    def _score_receive_phase(self, state, player: str, action_type: str, block: Optional[str]) -> float:
        if action_type == "pass":
            base = 0.0
            if self._would_lose_if_pass(state, player):
                return -1e12
        else:
    ...
    def select_action(self, state, player: str, actions: List[Action]) -> Action:
        ...
        if tr is not None and tr.get("shi_plan_eligible", False):
            ...
            if (not tr.get("shi_plan_active", False)) and enemy_attack_turn and (not tr.get("first_enemy_attack_seen", False)):
                ...
                for act in actions:
                    if act[0] == "pass":
                        if not self._would_lose_if_pass(state, player):
                            return act
            ...
            if state.phase == "receive" and state.current_attack == "1" and state.attacker == ally and tr.get("shi_chain_passed", False):
                ...
                if my_shi == 3:
                    # パスが即負けに繋がる局面（相手リーチ等）では絶対にパスしない
                    if not self._would_lose_if_pass(state, player):
                        for act in actions:
                            if act[0] == "pass":
                                return act
