...
- Free accounts can save and import up to 20 records after registration. Active paid accounts can save and import up to 1,000 records. Both require a completed initial password change when one is pending; expired paid accounts retain existing records but cannot add new ones.
- Reading, editing, exporting, and deleting existing records remain available after paid access expires.
- The applicable limit is enforced inside the write transaction: 20 for free accounts and 1000 for active paid accounts. The library displays the current count and limit.
...
- My Page > Kifu Library groups automatic saving, manual saving and file import under “棋譜をサーバーに保存”. Automatic saving is expanded initially; manual saving and import are collapsed. The import button is inside the import disclosure, separate from the record-list filter.
- Automatic saving retains player names and is off by default (expanding its section does not enable the checkbox). Consent is stored per member in `member_kifu_settings` and can be disabled after paid access expires. Free members can enable it within the 20-record limit; active paid members can enable it within the 1000-record limit. Enabling requires a completed initial password change.
- My Page opens from the lobby/room menu without the settings tabs. Its Account and Kifu Library tabs remain available. Opening My Page or entering the library resets the disclosures to their initial states. The Statistics section, including its button and results, appears below the record list.
- Reopening My Page while Kifu Library was selected resets any record detail/replay and fetches the latest list, including rounds automatically saved while the page was closed. Ordinary session refreshes do not reset library navigation. Reopening from Account keeps that tab selected.
- At round completion, the server checks the current human seat owners against same-origin room WebSocket connections. Only a connected, logged-in, opted-in member with save entitlement is saved. Spectators, departed/replaced players and AI seats are excluded. A player who joins during a round and remains seated at the end is eligible; temporary disconnection at the instant of completion is not. There is no retrospective save when turning the setting on after a round.
...
- At the applicable limit (20 for free members, 1000 for active paid members), automatic saving is disabled; no existing record is removed. Other storage failures do not interrupt play. Private status notifications display failure/limit messages in the room and My Page; missed notifications do not undo a committed save. There is no background retry queue storing unsaved kifu.
