...
- Automatic saving retains player names and is off by default (expanding its section does not enable the checkbox). Consent is stored per member in `member_kifu_settings` and can be disabled after paid access expires. Enabling requires active paid access and a completed initial password change.
