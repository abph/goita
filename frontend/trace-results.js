...
function updatePracticeReplayButtons(state, isHost, autoNextRoundPending) {
  ...
  const sceneButton = document.getElementById('btnPracticeScene');
  const exitButton = document.getElementById('practiceExitButton');
  if (!replayButton || !returnButton || !sceneButton || !exitButton) return;
  ...
  const canReplay = (
    ...
    && !isScoreAttackRoom()
    && PRIVATE_ROOM_IDS.has(gid)
  );
  replayButton.style.display = canReplay && !active ? '' : 'none';
  replayButton.textContent = uiText('もう一度練習する');
  replayButton.onclick = () => practiceReplayAction(active ? 'retry' : 'start');
  const canReturn = isHost && active && PRIVATE_ROOM_IDS.has(gid) && !isScoreAttackRoom();
  returnButton.style.display = canReturn ? '' : 'none';
  returnButton.textContent = uiText('元のゲームに戻る');
  returnButton.onclick = returnFromPracticeReplay;
  exitButton.style.display = canReturn ? 'grid' : 'none';
  exitButton.onclick = returnFromPracticeReplay;
  ...
  exitButton.disabled = practiceReplayRequestInFlight || traceActionBusy;
  sceneButton.disabled = practiceReplayRequestInFlight || traceActionBusy;
}
