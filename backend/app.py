...
def _run_cpu_until_human(game: Dict[str, Any]) -> None:
    ...
    # 既に終了している場合は何もしない（finishedログの重複防止）
    if state.finished:
        return
    ...
    if state.finished:
        # finishedログは1回だけ
        if not any(str(x).startswith("Game finished.") for x in log[-5:]):
            log.append(f"Game finished. winner={state.winner}, team_score={getattr(state, 'team_score', None)}")def _create_game_obj(dealer: str = "A") -> Dict[str, Any]:
