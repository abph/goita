...
class MemberKifuStore:
    ...
    def access(self, token, record_id, *, action="get", title="", memo="", tags=None):
        with self.members._db(write=action != "get") as db:
            ...
            if action == "delete":
                ...
                return None
            if action == "edit":
                db.execute("UPDATE member_kifu SET title = ?, memo = ?, tags_json = ? WHERE member_id = ? AND id = ?", (
                    title.strip() or row["title"], memo.strip(),
                    row["tags_json"] if tags is None else json.dumps(tags, ensure_ascii=False), owner, record_id,
                ))
                row = db.execute("SELECT * FROM member_kifu WHERE member_id = ? AND id = ?", (owner, record_id)).fetchone()
