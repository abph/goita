...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#traceOriginalScore').textContent(),'元の棋譜 BD：30点');
    assert.equal(await page.locator('#traceActualScore').textContent(),'今回 AC：40点');
    ...
    assert.equal(await page.locator('#traceActualScore').textContent(),'今回 AC：30点');
    assert.equal(await page.locator('#traceOriginalScore').textContent(),'元の棋譜 AC：30点');
    assert.equal(await page.locator('#traceImprovement').textContent(),'点差0点');
