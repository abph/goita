...
def test_language_switcher_and_translation_runtime_are_loaded() -> None:
    ...
    assert '<script src="/static/i18n-en.js?v=20260917d"></script>' in html
    assert '<script src="/static/i18n.js?v=20260917d"></script>' in html
...
def test_lobby_feature_guide_is_translated() -> None:
    ...
    assert "日本ごいた協会認定　オンライン対局室へようこそ。" not in html
    assert "日本ごいた協会認定" in html
    assert "オンライン対局室" in html
...
def test_practice_replay_controls_are_translated() -> None:
    chinese = (ROOT / "frontend" / "i18n.js").read_text(encoding="utf-8")
    english = (ROOT / "frontend" / "i18n-en.js").read_text(encoding="utf-8")
