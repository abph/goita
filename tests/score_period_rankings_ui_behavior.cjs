...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.lobby-toolbar .lobby-menu-label').isVisible(),true);
    const guide = await page.locator('.lobby-purpose-guide').boundingBox();
    assert.equal(await page.locator('#lobbyPurposeHome .lobby-purpose-desktop-label').first().isVisible(),true);
    assert.equal(await page.locator('#lobbyPurposeHome .lobby-purpose-mobile-label').first().isVisible(),false);
    ...
    assert.equal(await page.locator('#lobbyPublicRoomsPanel').isVisible(),true);
    await page.evaluate(()=>showLobbyPurposeStep('study'));
    await page.locator('#lobbyPurposeStudy .lobby-purpose-option').first().click();
    assert.equal(await page.locator('#lobbyPurposeDeal').getAttribute('aria-hidden'),'false');
    assert.equal(await page.locator('#lobbyPurposeDealHeader').getAttribute('aria-hidden'),'false');
    assert.match(await page.locator('#lobbyPurposeDeal').textContent(),/極端な強弱を除いて練習する/);
    await page.locator('#lobbyPurposeDealHeader .lobby-purpose-back-arrow').click();
    assert.equal(await page.locator('#lobbyPurposeStudy').getAttribute('aria-hidden'),'false');
    ...
    assert.ok(rank.x >= dailyRanking.x);
    assert.ok(Math.abs(rank.y - play.y) < 1);
    assert.ok(Math.abs(rank.height - play.height) < 1);
    ...
    holdDaily = false;
    await page.evaluate(()=>showLobbyPurposeStep('home'));
    await page.setViewportSize({width:390,height:700});
    const mobileBrand = await page.locator('.lobby-brand-lockup').boundingBox();
    const mobileMenu = await page.locator('.lobby-toolbar .header-menu-trigger').boundingBox();
    assert.equal(await page.locator('.lobby-toolbar .lobby-menu-label').isVisible(),false);
    assert.ok(mobileBrand.x + mobileBrand.width < mobileMenu.x);
    ...
    assert.equal(await page.locator('#lobbyPurposeHome .lobby-purpose-desktop-label').first().isVisible(),false);
    assert.equal(await page.locator('#lobbyPurposeHome .lobby-purpose-mobile-label').first().isVisible(),true);
    assert.deepEqual(await page.locator('#lobbyPurposeHome .lobby-purpose-mobile-label').allTextContents(),['対局したい','研究したい']);
    assert.equal(await page.locator('#lobbyPurposeHome .lobby-purpose-option > span').evaluateAll(items=>items.some(item=>item.offsetParent!==null)),false);
    assert.equal(await page.locator('#scoreAttackEntry').textContent(),'挑戦する');
    assert.equal(await page.locator('.score-attack-description-desktop').isVisible(),false);
    assert.equal(await page.locator('.score-attack-description-mobile').isVisible(),true);
    assert.equal(await page.locator('.score-attack-description-mobile').textContent(),'同じ手駒・親でAIと対戦し、元の棋譜を超えてランキング上位を目指します。');
    assert.ok(Math.abs(mobileScore.y-mobileGuide.y) < 2);
    assert.ok(mobileScore.x-(mobileGuide.x+mobileGuide.width) >= 6);
    assert.ok(Math.abs(mobileScore.width-mobileGuide.width) < 2);
    assert.ok(Math.abs(mobileScore.height-mobileGuide.height) < 2);
    const mobilePlay = await section.locator('#scoreAttackEntry').boundingBox();
    const mobileRank = await section.locator('#scoreRankingsEntry').boundingBox();
    assert.ok(mobileRank.y > mobilePlay.y);
    ...
    assert.equal(await page.locator('#scoreAttackDailyRankingList li').count(),3);
    assert.equal(await page.locator('#scoreAttackDailyRankingList .score-attack-daily-placeholder').count(),3);
